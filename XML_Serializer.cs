...
namespace Combined_XML_Program
{
    class XML_Serializer
    {
        ...
        private static void SpawnListXML(string? basePath, string? typePath)
        {
            string[] fileEntries = Directory.GetFiles(basePath + typePath + @"\", "*.xml");
            foreach (var entry in fileEntries)
            {
                var Xml = SS_Serializer_SpawnList.GALAXY.LoadFromFile(entry);
                var s1 = entry.Replace(basePath, "");
